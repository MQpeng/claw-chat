...
void main() async {
  ...
  channel.stream.listen(
    (data) {
      ...
      if (frame['type'] == 'event' && frame['event'] == 'connect.challenge') {
        print('<<< Got connect.challenge, sending connect (fixed client id/mode)...');
        final request = {
          'type': 'req',
          'id': 'connect-auth',
          'method': 'connect',
  ...
  try {
    ...
    if (result == 'pairing required') {
      ...
      print('\n⚠️  Got different error message, but protocol works.');
      print('Gateway returns error message correctly.');
    }
  } catch (e) {
    print('\n❌ Error: $e');
  } finally {
